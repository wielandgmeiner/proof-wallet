...
export PROOFWALLET=env COVERAGE_FILE=../../coverage/$(coverfile) python3-coverage run ../../proofwallet.py
...
ifdef COVERAGE
	@which python3-coverage > /dev/null || (echo 'Error: unable to find coverage (Maybe "pip3 install coverage"?)'; exit 1)
	@rm -rf coverage
	@mkdir -p coverage
endif
